...
class DailyExcel extends Command
{
    ...
    private function createExcel()
    {
        ...
        $restaurantOrders = Order::where('afhaaltijdstip', '=', null)->with('orders')
            ->with('orders.gerecht')
            ->with('orders.aanbieding')
            ->get();

        $takeAwayOrders = Order::where('afhaaltijdstip', '!=', null)->with('orders')
            ->with('orders.gerecht')
            ->with('orders.aanbieding')
            ->get();
    ...
    private function addOrdersToSheet($sheet, $orders)
    {
        ...
        foreach ($orders as $index => $order) {
            ...
            if ($cellIndex - 3 > $highestCellIndex) $highestCellIndex = $cellIndex - 3;
        }
